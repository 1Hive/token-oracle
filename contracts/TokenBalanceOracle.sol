...
contract TokenBalanceOracle is AragonApp, IACLOracle {
    ...
    string private constant TOKEN_BALANCE_CANT_PERFORM = "TB_CANT_PERFORM";
    ...
    */
    function canPerform(address, address, bytes32, uint256[] _how) external view returns (bool) {
        require(_how.length > 0 && _how[0] < 2**160 && _how[0] != 0, TOKEN_BALANCE_CANT_PERFORM);
        return token.balanceOf(address(_how[0])) >= minBalance;
    }
}
